...
def neuralSkip(x, y, numFilters, kernelSize, stride, actFunc, w_init, b_init, name):
    m = Conv2d(x, numFilters // 2, (1, 1), (stride, stride), act=actFunc, padding='SAME', W_init=w_init, b_init=b_init,
               name=f'k{kernelSize}n{numFilters}s{stride}/c1/nskip{name}')
    m = Conv2d(m, numFilters * 2, (1, 1), (stride, stride), act=None, padding='SAME', W_init=w_init, b_init=b_init,
               name=f'k{kernelSize}n{numFilters}s{stride}/c2/nskip{name}')
    m = Conv2d(m, numFilters, (kernelSize, kernelSize), (stride, stride), act=tf.nn.tanh, padding='SAME', W_init=w_init,
               name=f'k{kernelSize}n{numFilters}s{stride}/c3/nskip{name}')
    # y = y+m*x
...
def subPixelSkip(shallow, deep, numFilters, kernelSize, stride, actFunc, w_init, b_init, name):
    shallow = Conv2d(shallow, numFilters, (kernelSize, kernelSize), (stride, stride), act=actFunc, padding='SAME',
                     W_init=w_init, b_init=b_init, name=f'k{kernelSize}n{numFilters}s{stride}/c1/nskip{name}')
...
def residualBlock(n, numFilters, kernelSize, stride, batchNorm, actFunc, w_init, b_init, g_init, i):
    nn = Conv2d(n, numFilters, (kernelSize, kernelSize), (stride, stride), act=actFunc, padding='SAME', W_init=w_init,
                b_init=b_init, name=f'k{kernelSize}n{numFilters}s{stride}/c1/{i}')
    nn = BatchNormLayer(nn, act=actFunc, is_train=batchNorm, gamma_init=g_init,
                        name=f'k{kernelSize}n{numFilters}s{stride}/b1/{i}')
    nn = Conv2d(nn, numFilters, (kernelSize, kernelSize), (stride, stride), act=None, padding='SAME', W_init=w_init,
                b_init=b_init, name=f'k{kernelSize}n{numFilters}s{stride}/c2/{i}')
...
def generator2D(t_image, scaleFactor, numResBlocks, numFilters, kernelSize, initKernelFactor, activation,
                batchNorm=True, reuse=False):
    """ SRGAN generator"""
    stride = 1
    ...
    with tf.variable_scope("SRGAN_g", reuse=reuse) as vs:
        ...
        n = Conv2d(n, numFilters, (kernelSize * initKernelFactor, kernelSize * initKernelFactor), (1, 1), act=None,
                   padding='SAME', W_init=w_init, name=f'k{kernelSize * initKernelFactor}n{numFilters}s{stride}/cInit')
        shallow = n
        # B residual blocks
        ...
        n = Conv2d(n, numFilters, (kernelSize, kernelSize), (stride, stride), act=None, padding='SAME', W_init=w_init,
                   b_init=b_init, name=f'k{kernelSize}n{numFilters}s{stride}/c/m')
        ...
        n = Conv2d(n, subPixConvFilters, (kernelSize, kernelSize), (stride, stride), act=None, padding='SAME',
                   W_init=w_init, name=f'k{kernelSize}n{subPixConvFilters}s{stride}/1')
        ...
        n = Conv2d(n, subPixConvFilters, (kernelSize, kernelSize), (stride, stride), act=None, padding='SAME',
                   W_init=w_init, name=f'k{kernelSize}n{subPixConvFilters}s{stride}/2')
...
def generator2DWide(t_image, scaleFactor, numResBlocks, numFilters, kernelSize, batchNorm=False, reuse=False):
    ...
    with tf.variable_scope("SRGAN_g", reuse=reuse) as vs:
        ...
        # B residual blocks
        for i in range(16):
            nn = Conv2d(n, 64 * 6, (1, 1), (1, 1), act=lrelu, padding='SAME', W_init=w_init, b_init=b_init,
                        name='n64s1/c1/%s' % i)
            nn = BatchNormLayer(nn, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='n64s1/b1/%s' % i)
            nn = Conv2d(nn, 48, (1, 1), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init,
                        name='n64s1/c2/%s' % i)
        ...
        shallow = Conv2d(shallow, 3 * 4 ** 2, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init,
                         name='n64s1/c/m')
        ...
        deep = Conv2d(deep, 3 * 4 ** 2, (5, 5), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init,
                      name='n256s1/1')
...
def discriminator2D(input_images, is_train=True, reuse=False):
    ...
    with tf.variable_scope("SRGAN_d", reuse=reuse):
        ...
        for i in range(numDiscrimBlocks):
            expon = 2 ** (i + 1)
            # expand x2
            net = Conv2d(net, df_dim * expon, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init,
                         name=f'db{i}/h1/c')
            net = BatchNormLayer(net, act=lrelu, is_train=is_train, gamma_init=gamma_init, name=f'db{i}/h1/bn')
            # upsample
            net = Conv2d(net, df_dim * expon, (3, 3), (2, 2), act=None, padding='SAME', W_init=w_init,
                         name=f'db{i}/h2/c')
