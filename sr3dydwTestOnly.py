...
def argumentParser():
    ...
    parser.add_argument('Input_Images', metavar='Input Images', help='Folder containing input images',
                        widget="DirChooser")
    ...
    parser.add_argument('Scale_Factor', metavar='Scale Factor', type=str, choices=['4', '16', '64'],
                        help='Upsampling factor (Future releases will support more options)')

    parser.add_argument('Use_3D_Patches', metavar='Use 3D Patches', type=str, choices=['yes', 'no'],
                        help='Use Patch based super resolution for 3D images (recommended)')

    parser.add_argument('Output_Format', metavar='Output Format', type=str,
                        choices=['same', '.png', '.jpg', '.mat', '.nc', '.tif'])
    ...
    parser.add_argument('Model_Type', metavar='Model Type', type=str,
                        choices=['2D Rock', '2D Augmented Rock', '3D Rock', '2D Photo'],
                        help='Dimensions of input images')

    parser.add_argument('Use_CPU', metavar='Use CPU', type=str, choices=['yes', 'no'],
                        help='Force the network to use the CPU (will default to CPU if no compatible GPU is detected)',
                        default='yes')

    parser.add_argument('Use_GAN', metavar='Use GAN', type=str, choices=['yes', 'no'],
                        help='Force the network to use the GAN (will default to CNN)', default='no')
...
def main():
    ...
    if dim == '2D Photo':
        ...
        if ganFlag:
            ...
            #restore = './validatedCheckpoints/epoch-1-PSNR-15.439708164522479.ckpt'
            #restore = './validatedCheckpoints/epoch-31-PSNR-20.708941956186884.ckpt'
    ...
    if flatFlag:  # 2D EDSR model (built on TL)
        net_gen = generator2D(inputTensor, Scale_Factor, residual_blocks, numFilters, 3, initKernelFactor=1,
    ...
    for fname in files:
        if os.path.isfile(fname):
            ...
            if lr.max() > 255 or lr.min() < 0.0:
                ...
                print(
                    'WARNING: image is not within network bounds, performing automatic contrast adjustment (assuming inscribed cylinder) to ' + str(
                        truncMax) + ' - ' + str(truncMin))
            ...
            for n in range(numLoops):
                if flatFlag and lr.shape[3] > 3:
                    ...
                    temp = np.zeros(
                        (lr.shape[0], lr.shape[1] * Scale_Factor, lr.shape[2] * Scale_Factor, lr.shape[3], lr.shape[4]))
                    ...
                    temp2 = np.zeros(
                        (temp.shape[0], temp.shape[1], temp.shape[2] // Scale_Factor, temp.shape[3], temp.shape[4]))
                    for i in range(temp.shape[3]):
                        print("\rCompressing Slice %d" % (i + 1))
                        compSlice = np.array(Image.fromarray(np.squeeze(temp[:, :, :, i, :])).resize(
                            (temp.shape[1], temp.shape[2] // Scale_Factor), PIL.Image.BICUBIC))
                        temp2[:, :, :, i] = np.expand_dims(np.expand_dims(compSlice, 0), 3)
                    #                    stdout.flush()
                    #                    stdout.write("\n")
                    del temp
                    temp2 = (temp2 - numBits) / numBits
                    sr3 = np.zeros((lr.shape[0], lr.shape[1] * Scale_Factor, lr.shape[2] * Scale_Factor,
