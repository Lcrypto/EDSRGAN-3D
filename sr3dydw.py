...
def loadDataset(iterNum, batch_size, trainImageIDs, img_width, img_height, img_depth, scaleFactor, path, name, subset,
                downgrade, datasetBitDepth, numBits):
    ...
    if img_depth > 1:
        ...
        if scaleFactor == 'M':
            ...
            lr_batch = np.zeros(
                (len(trainBatchIDs), img_width // scaleFactor, img_height // scaleFactor, img_depth // scaleFactor),
        ...
        if scaleFactor == 'M':
            ...
            lr_batch = np.zeros((len(trainBatchIDs), img_width // scaleFactor, img_height // scaleFactor, 3),
                                dtype=datasetBitDepth)
        ...
        if img_depth > 1:
            if scaleFactor != 'M':
                ...
                lr = lr[lr_w:lr_w + img_width // scaleFactor, lr_h:lr_h + img_height // scaleFactor,
                     lr_d:lr_d + img_depth // scaleFactor]
...
def _hr_image_path(path, name, subset, id):
    return os.path.join(path, f'{name}_{subset}_HR',
                        f'{id:04}.npy')  # TODO the 4 trailing zeros is legacy and I CBF removing it because itll fuck up all the other datasets
...
def argumentParser():
    ...
    parser.add_argument('--preprocess', type=str2bool, default=False,
                        help='Flag to activate if mat volumes need to be numpy-ified')
    ...
    parser.add_argument('--indir', type=str, default='../GAN_PhysRock&Roll/DRSRD3/DRSRD3_3D/shuffled3D', help='path to DIV2K images')

    parser.add_argument('--outdir', type=str, default='../GAN_PhysRock&Roll/shuffled3D_BIN',
    ...
    parser.add_argument('--classificationRatio', type=str2float, default=0.1,
                        help='target discriminatory ratio')  # if this is negative, tune against Dloss = 1

    parser.add_argument('--hyperFlag', type=str2bool, default=False,
                        help='Dynamic scaling of the Adversarial and Discriminatory Coefficients')

    parser.add_argument('--hyperBCFlag', type=str2bool, default=False,
                        help='Bicubic scaling of the Adversarial and Discriminatory Coefficient')
    ...
    parser.add_argument('--dataset', type=str, default='../GAN_PhysRock&Roll/Shuffled3D_BIN', help='path to dataset')
    ...
    parser.add_argument('--valVisInterval', type=str2int, default=10,
                        help='visualisation during training and validation')
...
def main():
    ...
    if trainFlag or testFlag:
        ...
        if args.multiScale:  # 3D multiscale SRCNN model (deprecated?)
            net_gen = generatorMS(input_gen=inputTensor, kernel=3, nb=residual_blocks, numFilters=numFilters,
                                  is_train=True, reuse=False)
        elif flatFlag:  # 2D EDSR model (built on TL)
            net_gen = generator2D(inputTensor, scaleFactor, residual_blocks, numFilters, 3, initKernelFactor=1,
                                  activation=activation, batchNorm=batchNorm, reuse=False)
        ...
        if glcmRatio > 0:
            ...
            numLevels = 8
            span = 2  # scaleFactor
            glcmLoss = glcmRatio * tf.reduce_mean(
                tf.abs(compute8WayGLCM(net_gen, numLevels, span) - compute8WayGLCM(t_target_image, numLevels, span)),
                name='GLCMGeneratorLoss')
        else:
            glcmLoss = tf.zeros(1, tf.float32)
        # calculate gradients to calculate the gradient discrepancy (guides the GAN)
        ...
        if gdRatio > 0:
            ...
            if flatFlag:  # channels might cause issues here?
                ...
                gd_loss = gdRatio * (tf.reduce_mean(tf.square(tf.abs(dx_real) - tf.abs(dx_fake))) + tf.reduce_mean(
                    tf.square(tf.abs(dy_real) - tf.abs(dy_fake))))
            else:
                dx_real = t_target_image[:, 1:, :, :, :] - t_target_image[:, :-1, :, :, :]
                dy_real = t_target_image[:, :, 1:, :, :] - t_target_image[:, :, :-1, :, :]
                ...
                gd_loss = gdRatio * (tf.reduce_mean(tf.square(tf.abs(dx_real) - tf.abs(dx_fake))) + tf.reduce_mean(
                    tf.square(tf.abs(dy_real) - tf.abs(dy_fake))) + tf.reduce_mean(
                    tf.square(tf.abs(dz_real) - tf.abs(dz_fake))))
        else:
        ...
        if perceptFlag and trainFlag:
            ...
            vgg_loss = gRatio * tl.cost.mean_squared_error(vgg_predict_emb.outputs, vgg_target_emb.outputs,
                                                           is_mean=True)
        ...
        if ganFlag and trainFlag:
            ...
            if flatFlag:
                ...
                disc_out_real, logits_real = discriminatorTF(input_disc=t_target_image, kernel=3, is_train=True,
                                                             reuse=False)
            ...
            with tf.variable_scope('Adversarial_loss'):
                g_gan_loss = advRatio * tl.cost.sigmoid_cross_entropy(logits_fake, tf.ones_like(logits_fake),
                                                                      name='g')  # tf.reduce_mean(-tf.log(disc_out_fake+eps))
            ...
            gMSE_optim = tf.train.AdamOptimizer(learning_rate).minimize(mse_loss, var_list=g_vars,
        ...
        if trainFlag:
            ...
            if diskFlag is not True:
                ...
                trainLR, trainHR = loadDataset(0, len(trainImageIDs), trainImageIDs, img_width, img_height, img_depth,
                                               scaleFactor, path, name, 'train', downgrade, datasetBitDepth, numBits)

                valLR, valHR = loadDataset(0, len(valImageIDs), valImageIDs, valW, valH, valD, scaleFactor, path, name,
                                           'valid', downgrade, datasetBitDepth, numBits)
            ...
            for epochNum in range(val_restore, epochs + val_restore):
                ...
                for iterNum in range(iterations_train):
                    # load training batch
                    if diskFlag:
                        lr_train, hr_train = loadDataset(iterNum, batch_size, trainImageIDs, img_width, img_height,
                                                         img_depth, scaleFactor, path, name, 'train', downgrade,
                                                         datasetBitDepth, numBits)
                    else:
                    ...
                    if ganFlag:
                        if epochNum < ganEpoch:  # run mse and glcm only, no vgg or gan
                            # update G
                            errglcm, errmse, tpsnr, learn_Rate, _ = session.run(
                                [glcmLoss, mse_loss, g_psnr, learning_rate, gMSE_optim],
                                {inputTensor: lr_train, t_target_image: hr_train})
                            batchPSNR = np.mean(tpsnr)
                            stdout.write(
                                "\rLR: {%.4e} Epoch [%4d/%4d] [%4d/%4d]: glcmloss: %.4e (mse: %.4e) [psnr: %.4f]" % (
                                learn_Rate, epochNum + 1, epochs + val_restore, iterNum + 1, iterations_train, errglcm,
                                errmse, np.mean(tpsnr)))
                        else:
                            # update G and generate fakes
                            errg, errglcm, errmse, errgan, errgd, errvgg, tpsnr, learn_Rate, alpha, beta, _ = session.run(
                                [g_loss, glcmLoss, mse_loss, g_gan_loss, gd_loss, vgg_loss, g_psnr, learning_rate,
                                 gRatio, advRatio, g_optim], {inputTensor: lr_train, t_target_image: hr_train})

                            # update D by feeding it reals and fakes
                            errd, errdR, errdF, _ = session.run([d_loss, disc_out_real, disc_out_fake, d_optim],
                                                                {t_target_image: hr_train, inputTensor: lr_train})
                            batchPSNR = np.mean(tpsnr)
                            stdout.write(
                                "\rLR: {%.4e} Epoch [%4d/%4d] [%4d/%4d]: d_loss: %.4f dR: %.4f dF: %.4f g_loss: %.4e (mse: %.4e glcm: %.4e gdl: %.4e vgg: %.4e adv: %.4e) [psnr: %.4f]" % (
                                learn_Rate, epochNum + 1, epochs + val_restore, iterNum + 1, iterations_train, errd,
                                np.mean(errdR), np.mean(errdF), errg, errmse, errglcm, errgd, errvgg, errgan,
                                np.mean(tpsnr)))
                        ...
                        # update G
                        errglcm, errmse, tpsnr, learn_Rate, _ = session.run(
                            [glcmLoss, mse_loss, g_psnr, learning_rate, g_optim],
                            {inputTensor: lr_train, t_target_image: hr_train})
                        batchPSNR = np.mean(tpsnr)
                        stdout.write(
                            "\rLR: {%.4e} Epoch [%4d/%4d] [%4d/%4d]: glcmLoss: %.4e (mse: %.4e) [psnr: %.4f]" % (
                ...
                stdout.write("\n")
                print('Mean metrics: MSE: %.4e, GLCM: %.4e, PSNR: %.4f, VGG: %.4e, ADV: %.4e, DLOSS: %.4f' % (
                np.mean(trainingMSE), np.mean(trainingGLCM), np.mean(trainingPSNR), np.mean(trainingVGG),
                np.mean(trainingADV), np.mean(trainingDLOSS)))
                ...
                if np.mod(epochNum + 1,
                          valVisInterval) == 0 or epochNum == val_restore or epochNum == val_restore + epochs:
                ...
                for n in range(numValIterations):
                    if diskFlag:
                        lr_val, hr_val = loadDataset(n, 1, valImageIDs, valW, valH, valD, scaleFactor, path, name,
                                                     'valid', downgrade, datasetBitDepth, numBits)
                    else:
                    ...
                    if np.mod(epochNum + 1,
                              valVisInterval) == 0 or epochNum == val_restore or epochNum == val_restore + epochs:
                        imsave(f'{trainOutputDir}/epoch-{(epochNum + 1):04}/{n + 1:04}.tif',
                               np.array(np.squeeze(superRes.astype('int')), dtype=datasetBitDepth))
                ...
                if np.mod(epochNum + 1,
                          valVisInterval) == 0 or epochNum == val_restore or epochNum == val_restore + epochs or meanValPSNR > oldMaxValPSNR:
                    if not flatFlag:
        ...
        if testFlag:  # piggy back of the variables
            if flatFlag:
                ...
                for path in pngs:
                    ...
                    imsave(f'{outdir}/{name}-sr{ganStr}.tif',
                           np.array(np.squeeze(sr.astype('int')), dtype=datasetBitDepth))
