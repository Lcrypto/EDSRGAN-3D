...
def conv3dydw(x, numInFilters, numFilters, kernelSize, stride, name, reuse, trainable, padding, use_bias,
              kernel_initializer):
...
def generatorTF(inputs, kernelSize=3, numResidualBlocks=16, numFilters=64, scaleFactor=4, reuse=False, isTrain=True):
    ...
    def residual_block(inputs, numFilters, kernelSize, stride, scope):
        with tf.variable_scope(scope):
            net = conv3dydw(inputs, numFilters, numFilters, kernelSize, stride, name='Conv1', reuse=reuse,
                            trainable=isTrain, padding='same', use_bias=biasFlag, kernel_initializer=kernelInit)

            net = activate(net, activation)

            net = conv3dydw(net, numFilters, numFilters, kernelSize, stride, name='Conv2', reuse=reuse,
                            trainable=isTrain, padding='same', use_bias=biasFlag, kernel_initializer=kernelInit)
    ...
    with tf.variable_scope('SRGAN_g', reuse=reuse):
        ...
        with tf.variable_scope('initialFilter'):
            ...
            x = tf.keras.layers.Conv3D(input_shape=(None, None, None, None, 1), filters=numFilters,
                                       kernel_size=kernelSize, strides=stride, padding='same')(x)
        ...
        with tf.variable_scope('resBlockOutFilt'):
            x = conv3dydw(x, numFilters, numFilters, kernelSize, stride, name='convOut', reuse=reuse, trainable=isTrain,
                          padding='same', use_bias=biasFlag, kernel_initializer=kernelInit)
        ...
        with tf.variable_scope('subpixelconv_stage1'):
            x = conv3dydw(x, numFilters, numFilters * factor, kernelSize, stride, name='convSub1', reuse=reuse,
                          trainable=isTrain, padding='same', use_bias=biasFlag, kernel_initializer=kernelInit)
        ...
        with tf.variable_scope('subpixelconv_stage2'):
            x = conv3dydw(x, numFilters, numFilters * factor, kernelSize, stride, name='convSub2', reuse=reuse,
                          trainable=isTrain, padding='same', use_bias=biasFlag, kernel_initializer=kernelInit)
        ...
        with tf.variable_scope('output_stage'):
            x = conv3dydw(x, numFilters, 1, kernelSize, stride, name='convLast', reuse=reuse, trainable=isTrain,
                          padding='same', use_bias=biasFlag, kernel_initializer=kernelInit)
...
def generatorTFWide(inputs, kernelSize=3, numResidualBlocks=16, numFilters=64, scaleFactor=4, reuse=False,
                    isTrain=True):
    stride = 1
    biasFlag = True
    kernelInit = None
    ...
    def residual_block(inputs, numFilters, kernelSize, stride, scope):
        with tf.variable_scope(scope):
            net = conv3dydw(inputs, numFilters, numFilters * 6, 1, stride, name='Conv1', reuse=reuse, trainable=isTrain,
                            padding='same', use_bias=biasFlag, kernel_initializer=kernelInit)

            net = activate(net, activation)

            net = conv3dydw(net, numFilters * 6, int(numFilters * 0.8), 1, stride, name='Conv2', reuse=reuse,
                            trainable=isTrain, padding='same', use_bias=biasFlag, kernel_initializer=kernelInit)

            net = conv3dydw(net, int(numFilters * 0.8), numFilters, kernelSize, stride, name='Conv3', reuse=reuse,
                            trainable=isTrain, padding='same', use_bias=biasFlag, kernel_initializer=kernelInit)
    ...
    with tf.variable_scope('SRGAN_g', reuse=reuse):
        ...
        with tf.variable_scope('initialFilter'):
            ...
            # x.uses_learning_phase = True
            x = tf.keras.layers.Conv3D(input_shape=(None, None, None, None, 1), filters=numFilters, kernel_size=3,
        ...
        with tf.variable_scope('subpixelconv_shallow'):
            x = conv3dydw(shallowOutput, numFilters, numFilters, kernelSize, stride, name='convSub1', reuse=reuse,
                          trainable=isTrain, padding='same', use_bias=biasFlag, kernel_initializer=kernelInit)
        ...
        with tf.variable_scope('subpixelconv_shallow2'):
            x = conv3dydw(x, numFilters, numFilters, kernelSize, stride, name='convSub1', reuse=reuse,
                          trainable=isTrain, padding='same', use_bias=biasFlag, kernel_initializer=kernelInit)
        ...
        with tf.variable_scope('subpixelconv_deep'):
            x = conv3dydw(deepOutput, numFilters, numFilters, 5, stride, name='convSub1', reuse=reuse,
                          trainable=isTrain, padding='same', use_bias=biasFlag, kernel_initializer=kernelInit)
        ...
        with tf.variable_scope('subpixelconv_deep2'):
            x = conv3dydw(x, numFilters, numFilters, 5, stride, name='convSub1', reuse=reuse, trainable=isTrain,
                          padding='same', use_bias=biasFlag, kernel_initializer=kernelInit)
        ...
        with tf.variable_scope('output_stage'):
            x = conv3dydw(x, numFilters, 1, 1, stride, name='convLast', reuse=reuse, trainable=isTrain, padding='same',
                          use_bias=biasFlag, kernel_initializer=kernelInit)
...
def generator(input_gen, kernel, nb, scaleFactor, reuse, numFilters):
    ...
    with tf.variable_scope("SRGAN_g", reuse=reuse):
        ...
        x = Conv3dLayer(x, shape=[kernel, kernel, kernel, 1, numFilters], strides=[1, 1, 1, 1, 1], padding='SAME',
                        name='Conv1')
        ...
        for i in range(nb):
            x = Conv3dLayer(x, shape=[kernel, kernel, kernel, numFilters, numFilters], strides=[1, 1, 1, 1, 1],
                            padding='SAME', act=lrelu1, name='ResBlock/%sConvA' % i)

            x = Conv3dLayer(x, shape=[kernel, kernel, kernel, numFilters, numFilters], strides=[1, 1, 1, 1, 1],
                            padding='SAME', name='ResBlock/%sConvB' % i)
        ...
        factor = 1
        # upscaling block 1
        x = Conv3dLayer(x, shape=[kernel, kernel, kernel, numFilters, numFilters * factor], act=lrelu1,
                        strides=[1, 1, 1, 1, 1], padding='SAME', name='conv1-ub/1')

        # x = pixelShuffler3D(x, 2)
        x = UpSampling3D(name='UpSampling3D_1')(x.outputs)

        x = Conv3dLayer(InputLayer(x, name='in ub1 conv2'),
                        shape=[kernel, kernel, kernel, numFilters * factor, numFilters * factor], act=lrelu1,
                        strides=[1, 1, 1, 1, 1], padding='SAME', name='conv2-ub/1')
        ...
        x = Conv3dLayer(InputLayer(x, name='in ub2 conv2'), shape=[kernel, kernel, kernel, numFilters * factor, 1],
                        strides=[1, 1, 1, 1, 1], act=tf.nn.tanh, padding='SAME', name='convlast')
...
def generatorMS(input_gen, kernel, reuse, nb, numFilters, is_train=True):
    ...
    with tf.variable_scope("SRGAN_g", reuse=reuse):
        ...
        for i in range(nb):
            x = Conv3dLayer(x, shape=[kernel, kernel, kernel, numFilters, numFilters], strides=[1, 1, 1, 1, 1],
                            padding='SAME', name='ResBlock/%sConvA' % i)
        ...
        x = Conv3dLayer(x, shape=[kernel, kernel, kernel, numFilters, 1], strides=[1, 1, 1, 1, 1], act=tf.nn.tanh,
                        padding='SAME', name='convlast')
...
def discriminatorTF(input_disc, kernel, is_train=True, reuse=False):
    ...
    def discriminator_block(inputs, numInFilters, numFilters, kernelSize, stride, scope, reuse):
        with tf.variable_scope(scope):
            net = conv3dydw(inputs, numInFilters, numFilters, kernelSize, stride, name='DConv1', reuse=reuse,
                            trainable=is_train, padding='same', use_bias=biasFlag, kernel_initializer=kernelInit)
...
def discriminator(input_disc, kernel, is_train=True, reuse=False):
    with tf.variable_scope("SRGAN_d", reuse=reuse):
        ...
        x = Conv3dLayer(x, act=lrelu2, shape=[kernel, kernel, kernel, 1, df_dim], strides=[1, 1, 1, 1, 1],
                        padding='SAME', name='conv1')

        x = Conv3dLayer(x, shape=[kernel, kernel, kernel, df_dim, df_dim], strides=[1, 2, 2, 2, 1], padding='SAME',
                        name='conv2')
        x = BatchNormLayer(x, is_train=is_train, name='BN1-conv2', act=lrelu2)

        x = Conv3dLayer(x, shape=[kernel, kernel, kernel, df_dim, df_dim * 2], strides=[1, 1, 1, 1, 1], padding='SAME',
                        name='conv3')
        x = BatchNormLayer(x, is_train=is_train, name='BN1-conv3', act=lrelu2)

        x = Conv3dLayer(x, shape=[kernel, kernel, kernel, df_dim * 2, df_dim * 2], strides=[1, 2, 2, 2, 1],
                        padding='SAME', name='conv4')
        x = BatchNormLayer(x, is_train=is_train, name='BN1-conv4', act=lrelu2)

        x = Conv3dLayer(x, shape=[kernel, kernel, kernel, df_dim * 2, df_dim * 4], strides=[1, 1, 1, 1, 1],
                        padding='SAME', name='conv5')
        x = BatchNormLayer(x, is_train=is_train, name='BN1-conv5', act=lrelu2)

        x = Conv3dLayer(x, shape=[kernel, kernel, kernel, df_dim * 4, df_dim * 4], strides=[1, 2, 2, 2, 1],
                        padding='SAME', name='conv6')
        x = BatchNormLayer(x, is_train=is_train, name='BN1-conv6', act=lrelu2)

        x = Conv3dLayer(x, shape=[kernel, kernel, kernel, df_dim * 4, df_dim * 8], strides=[1, 1, 1, 1, 1],
                        padding='SAME', name='conv7')
        x = BatchNormLayer(x, is_train=is_train, name='BN1-conv7', act=lrelu2)

        x = Conv3dLayer(x, shape=[kernel, kernel, kernel, df_dim * 8, df_dim * 8], strides=[1, 2, 2, 2, 1],
